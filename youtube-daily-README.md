...
어디까지 되고 어디서 막히는지 한 번에 확인합니다. 프록시를 붙인 뒤 자막이 실제로
뚫렸는지 확인할 때도 이걸 쓰면 됩니다.

```bash
python youtube_daily.py --diagnose
```

Actions에서는 **Run workflow → mode 를 `diagnose` 로 선택**하면 됩니다.
진단 모드에서는 요약을 만들지 않으므로 저장소에 커밋되는 것도 없습니다.

네 가지를 차례로 점검합니다.

| # | 항목 | 확인 내용 |
| --- | --- | --- |
| 1 | 요약 백엔드 | 구독 토큰 또는 API 키로 실제 호출이 되는지 |
| 2 | 프록시 | 설정 여부와 종류 |
| 3 | 채널 RSS | 등록된 채널을 각각 불러올 수 있는지 |
| 4 | 자막 | 최신 영상 하나로 자막을 실제로 받아올 수 있는지 |

자막 조회가 실패하면 프록시 설정 여부에 따라 다른 안내가 나옵니다 — 프록시가 없으면
붙이라고, 있는데도 막혔으면 자격 증명과 잔여 트래픽을 확인하라고 알려줍니다.
하나라도 실패하면 종료 코드 1이라 Actions에서 빨간불로 보입니다.
