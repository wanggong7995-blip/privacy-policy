...
| `transcript_extra_attempts` | 자막을 못 받았을 때 다음 후보로 몇 편 더 내려가 볼지 | 2 |
| `transcript_block_streak` | 몇 편 연속 차단되면 그 실행의 자막 요약을 접을지 | 2 |
...
- **자막이 하루치 통째로 막히는 날이 있습니다.** 집 IP에서도 유튜브가 가끔 자막 요청을 차단합니다
  (`IpBlocked`). 평소에는 9~10편이 요약되지만, 차단된 날에는 0편이 되고 브리핑이 기사 기반으로만
  만들어집니다. 그래서 이렇게 동작합니다.
  - 차단일 때만 10초 → 30초 → 60초로 다시 시도합니다. 자막이 아예 없는 영상은 기다리지 않고 넘어갑니다.
  - 한 영상이 실패하면 선별된 다음 영상으로 내려가며 목표 편수를 채웁니다.
  - 2편 연속 차단되면 그 실행의 자막 요약은 접습니다. 계속 두드려 봐야 차단만 길어지기 때문입니다.
  - 며칠 내리 차단된다면 프록시(`WEBSHARE_PROXY_USERNAME` 등, 유료)를 붙이는 것이 다음 수단입니다.
